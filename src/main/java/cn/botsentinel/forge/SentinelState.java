...
public final class SentinelState {
    ...
    // ---------- 生命周期 ----------
    public void init() {
        Path dir = FMLPaths.CONFIGDIR.get().resolve("BotSentinel");
        ...
        log("BotSentinel v2.4 (Forge) 已加载 | 评分: " + scoreEngine.engineName() + " | 模式: " + config.mode
    ...
    public void reload() {
        SentinelConfig fresh = SentinelConfig.load(
                FMLPaths.CONFIGDIR.get().resolve("BotSentinel").resolve("config.json"));
        this.config = fresh;
